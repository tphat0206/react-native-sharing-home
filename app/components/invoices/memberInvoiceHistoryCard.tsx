...
export default function MemberInvoiceHistoryCard({
    ...
    const [invoiceHistoryDetail, setInvoiceHistoryDetail] =
        useState<FullPersonalInvoiceHistory>();

    const handleShowExpense = () => {
        onShowExpense(invoice.id);
        setInvoiceHistoryDetail(dummy_room_member_invoice_history_detail);
    };
    ...
    return (
        <View className="bg-white my-1 rounded-2xl w-full px-4 py-4 shadow-md">
                ...
                <View className="flex-1">
                    ...
                    {!invoice.is_show_expense && (
                        <Text size="md" className="font-medium">
                            Actual cost: {invoice.actual_cost.toLocaleString()}
                        </Text>
                    )}
                </View>
            ...
            {invoice.is_show_expense ? (
                <TableMemberInvoiceHistory
                    expense={invoiceHistoryDetail?.invoice}
                />
            ) : null}
        </View>
    );
